...
function api(items, { falla = false, council = COUNCIL_FIXTURE } = {}) {
  ...
  const code = [functionSource("stockIndex"), functionSource("normalizaEtiqueta"),
    ...
    functionSource("ventanaReservada"), functionSource("sustituyePorCapsulas"),
  ...
  return new Function("fetch", "COUNCIL", "COUNCIL_ORDER", "STOCK_INDEX_URL",
    ...
    "COUNCIL_CAPSULA_TYPE", "COUNCIL_CAPSULA_TYPES", "COUNCIL_CAPSULA_MAX", "COUNCIL_KNOWLEDGE_PROMPT_CHARS",
    ...
    eval(constante("COUNCIL_KNOWLEDGE_DADO_SHARE")), eval(constante("COUNCIL_CAPSULA_TYPE")),
    new Set([eval(constante("COUNCIL_CAPSULA_TYPE")), "guion"]), Number(constante("COUNCIL_CAPSULA_MAX")), Number(constante("COUNCIL_KNOWLEDGE_PROMPT_CHARS")),
...
// ── CÁPSULAS: LO QUE APRENDE, NO EL TÍTULO ─────────────────────────────────

function capsula(dia, title, cuerpo, { fuente = "", tag = "stevejobs", legacy = false } = {}) {
  return { id: "ap" + dia, type: legacy ? "guion" : "capsula", createdAt: `2026-08-${String(dia).padStart(2, "0")}T12:00:00.000Z`,
    title, comment: cuerpo, externalRef: fuente, tags: [tag, "formacion"] };
}

test("de una cápsula el consejero lee el CUERPO, no el título", async () => {
  ...
  const a = api([capsula(6, "Dieter Rams: Less but Better", cuerpo, { tag: "dieterrams" })]);
  const [p] = await a.piezas("cdo");
  assert.equal(p.capsula, true);
  assert.equal(p.note, cuerpo, "el comentario de una cápsula ES el conocimiento, no el mecanismo");
  assert.match(a.texto([p]), /menos PERO MEJOR/, "y llega al prompt entero");
});

test("los guiones históricos se leen como cápsulas sin publicar nuevos con ese nombre", async () => {
  const [p] = await api([capsula(6, "Histórica", "Conocimiento legado", { legacy: true })]).piezas("ceo");
  assert.equal(p.capsula, true);
  assert.equal(p.note, "Conocimiento legado");
  assert.equal(eval(constante("COUNCIL_CAPSULA_TYPE")), "capsula");
});

test("un vídeo con cápsula deja de entrar: su título ya no añade nada", async () => {
  const video = pieza(5, "Dieter Rams: Less but Better", { formacion: true, tag: "dieterrams" });
  const nota = capsula(6, "Cápsula de Less but Better", "Quitar hasta que solo quede lo esencial.",
  ...
  assert.equal(todas[0].capsula, true);
  // …pero la cápsula conserva a QUÉ pieza apunta: el vídeo sigue siendo su fuente y
...
test("si la cápsula no declara fuente, basta con que se llamen igual", async () => {
  // Es como los sube quien transcribe: mismo título, tipo distinto.
  const video = pieza(5, "Jobs en Stanford", { formacion: true });
  const nota = capsula(6, "Jobs en Stanford", "Los puntos solo se unen mirando atrás.");
  const todas = await api([video, nota]).piezas("ceo", 0);
  assert.equal(todas.length, 1);
  assert.equal(todas[0].capsula, true, "gana la cápsula, no el vídeo");
});

test("una cápsula no se lleva por delante los vídeos de los que NO habla", async () => {
  const nota = capsula(6, "Jobs en Stanford", "Los puntos solo se unen mirando atrás.");
...
test("la ventana cuenta caracteres, no piezas: ocho cápsulas no son ocho títulos", async () => {
  // Ocho títulos son ~400 caracteres y caben; ocho cápsulas son ~5.000 y no.
  const largos = Array.from({ length: 8 }, (_, i) =>
    capsula(i + 1, "Cápsula " + i, "x".repeat(880)));
...
test("una cápsula enorme entra igual antes que dejar la ventana vacía", async () => {
  // Media idea es peor que una idea larga: una cápsula cortada no enseña nada.
  const top = await api([capsula(1, "Única", "y".repeat(5000))]).piezas("ceo");
...
test("el endpoint publica las cápsulas y distingue «ninguno largo» de «no se sabe»", () => {
  const ruta = source.slice(source.indexOf('url.pathname === "/council/knowledge"'),
    source.indexOf('url.pathname === "/ideas/generate"'));
  assert.match(ruta, /capsulas: pieces\.filter\(\(p\) => p\.capsula\)\.length/);
