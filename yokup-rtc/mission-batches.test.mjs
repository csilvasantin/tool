...
test('/fleet/informe acepta y deja esperando continuación sólo tras proof y firma válida', () => {
  ...
  assert.match(endpoint, /SELECT id,assignee,loc,status,source,screen,created_at,proof_image,proof_kind,live_shot,live_at,live_kind,live_surface,live_context,role FROM tickets/);
  assert.match(endpoint, /validateMissionProcessEvidence\(t\)/);
  assert.match(endpoint, /validateMissionActor\(t, owner\)/);
  assert.match(endpoint, /owner_mismatch[\s\S]*?applied: false/);
  assert.match(endpoint, /acceptBatchInformeClosure\(env, t, mid, owner, report\)/);
  assert.doesNotMatch(endpoint, /if \(crossSign\)/,
    'una firma cruzada se rechaza antes de escribir; no conserva informe ni proof');
