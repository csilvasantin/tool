...
test("la firma de generateCouncilIdea acepta el 3er parámetro topic", () => {
  assert.ok(SRC.includes("async function generateCouncilIdea(env, seat, topic)"));
});
...
test("la ruta POST /ideas/generate lee {topic} y lo pasa al generador", () => {
  ...
  assert.ok(SRC.includes("await generateCouncilIdea(env, seat, topic)"), "lo pasa al generador");
});
