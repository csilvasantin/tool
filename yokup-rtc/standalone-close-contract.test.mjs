...
test('informe primero completa atómicamente tarea standalone y ticket',()=>{
  const update=/UPDATE mission_tasks SET status='done',updated_at=\? WHERE mission_id=\? AND code!='z1' AND EXISTS\(SELECT 1 FROM tickets WHERE id=\? AND role='standalone-task'\)/;
...
test('misión standalone ya resuelta por contrato viejo acepta reparar z1 una vez',()=>{
  ...
  assert.match(informe,/UPDATE mission_tasks SET status='done',updated_at=\? WHERE mission_id=\? AND code!='z1'/);
});
