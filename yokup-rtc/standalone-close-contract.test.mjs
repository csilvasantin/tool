...
test('informe primero completa atómicamente tarea standalone y ticket',()=>{
  // La guarda `status!='no_aplica'` es parte del contrato desde el 1-sep-2026: el
  // informe final arrastra a `done` todo lo que quede suelto, y sin ella borraría
  // los pasos que un agente declaró «no aplicaba» — devolviendo al tablero la
  // misma mentira que ese estado vino a evitar.
  const update=/UPDATE mission_tasks SET status='done',report=COALESCE\(NULLIF\(TRIM\(report\),''\),\?\),ended_at=COALESCE\(ended_at,\?\),updated_at=\? WHERE mission_id=\? AND code!='z1' AND status!='done' AND status!='no_aplica' AND EXISTS\(SELECT 1 FROM tickets WHERE id=\? AND role='standalone-task'\)/;
...
test('misión standalone ya resuelta por contrato viejo acepta reparar z1 una vez',()=>{
  ...
  assert.match(informe,/UPDATE mission_tasks SET status='done',report=COALESCE\(NULLIF\(TRIM\(report\),''\),\?\),ended_at=COALESCE\(ended_at,\?\),updated_at=\? WHERE mission_id=\? AND code!='z1' AND status!='done' AND status!='no_aplica'/);
});
