...
test('task-status done con proof no cierra standalone antes del informe',()=>{
  assert.match(reconcile,/standalone = t\.role === "standalone-task"/);
  assert.match(reconcile,/hasInforme = tasks\.some/);
  assert.match(reconcile,/allDone && proof && \(!standalone \|\| hasInforme\)/);
  assert.match(reconcile,/blocked:"sin-informe"/);
});

test('informe primero completa atómicamente tarea standalone y ticket',()=>{
  const update=/UPDATE mission_tasks SET status='done',ended_at=COALESCE\(ended_at,\?\),updated_at=\? WHERE mission_id=\? AND code!='z1' AND status!='done' AND EXISTS\(SELECT 1 FROM tickets WHERE id=\? AND role='standalone-task'\)/;
...
test('misión standalone ya resuelta por contrato viejo acepta reparar z1 una vez',()=>{
  ...
  assert.match(informe,/UPDATE mission_tasks SET status='done',ended_at=COALESCE\(ended_at,\?\),updated_at=\? WHERE mission_id=\? AND code!='z1' AND status!='done'/);
});
