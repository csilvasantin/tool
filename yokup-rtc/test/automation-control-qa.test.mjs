...
test('SQLite: activación CLI reactivada sustituye APP sin duplicar la familia',async()=>{
  ...
  assert.equal(env.DB.raw.prepare('SELECT * FROM fleet_agent_modes WHERE identity_key=?').get(cli.key).mode,'training');
});
