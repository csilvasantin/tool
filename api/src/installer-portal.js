...
export async function handleInstaller(request,env,principal) {
 ...
 try {
  ...
  if(path==='/inbox' && method==='GET') {
   ...
   p.start_at AS appointment_start,p.end_at AS appointment_end,p.timezone AS appointment_timezone,p.scope AS appointment_scope,p.cost_cents AS appointment_cost_cents,CASE WHEN i.installer_id=? THEN d.address ELSE NULL END AS address
   FROM installer_notifications n JOIN installer_incidents i ON i.id=n.incident_id JOIN installer_devices d ON d.id=i.device_id LEFT JOIN retailer_incident_details rd ON rd.incident_id=i.id LEFT JOIN retailer_ratings rr ON rr.incident_id=i.id AND rr.installer_id=? LEFT JOIN call_cases cc ON cc.incident_id=i.id LEFT JOIN call_proposals p ON p.case_id=cc.id AND p.status='confirmed' AND p.installer_id=i.installer_id AND p.installer_id=n.installer_id WHERE n.installer_id=? ORDER BY n.created_at DESC LIMIT 100`,account.id,account.id,account.id);
