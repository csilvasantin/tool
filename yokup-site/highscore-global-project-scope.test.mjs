...
test("Highscore declara alcance global aunque la URL arrastre project_id",()=>{
  ...
  assert.match(frame,/PROJECT_SCOPE = globalProjectScopeSurface\(location\.pathname\) \? null : resolveProjectSurfaceScope\(projectId, "", PROJECT_CATALOG, policy\)/);
...
test("Highscore limpia sólo el query y conserva la preferencia de otras secciones",()=>{
  assert.match(frame,/var globalOnly = globalProjectScopeSurface\(location\.pathname\), policy=projectSurfacePolicy\(\)/);
  assert.match(frame,/if \(!globalOnly && \(!policy\.required \|\| policy\.canChange\)\) \{[\s\S]*localStorage\.setItem\(PROJECT_SCOPE_KEY, projectId\)/);
