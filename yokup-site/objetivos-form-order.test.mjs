...
test("el formulario conserva nombres accesibles y foco visible sin contaminar las acciones de la tabla", () => {
  for (const id of ["fTag", "fSeat", "fProject", "fTitle", "fBody"]) {
    assert.match(form, new RegExp(`id="${id}"[^>]*aria-label="[^"]+"`), `${id} tiene nombre accesible`);
  }
  assert.match(page, /\.add input:focus-visible,\.add textarea:focus-visible,\.add select:focus-visible,\.add button:focus-visible/);
  assert.equal((page.match(/\.objective-form-actions\{/g) || []).length, 1);
});
