...
function harness(saved){
  const store=storage(saved),buttons=[button("grid"),button("list")],listeners={};
...
test("primera visita nace en Cuadrícula sin grabar una preferencia implícita",()=>{
  const h=harness();
  const mounted=View.mount(h.container,{storage:h.store,target:h.target});
  assert.equal(mounted.getView(),"grid");
  assert.equal(h.target.attrs["data-informes-view"],"grid");
...
test("una elección posterior se restaura y sólo una activación explícita la cambia",()=>{
  ...
  h.listeners.click({target:h.buttons[0],preventDefault(){prevented++;},stopPropagation(){stopped++;}});
  assert.equal(mounted.getView(),"grid");
...
test("preferencias inválidas o storage no disponible caen de forma segura a Cuadrícula",()=>{
  assert.equal(View.read(storage({[View.STORAGE_KEY]:"cards"})),"grid");
  assert.equal(View.read({getItem(){throw new Error("blocked");}}),"grid");
  assert.equal(View.write({setItem(){throw new Error("quota");}},null,"list"),false);
});

test("el selector nombra ambas opciones y expone estado inequívoco",()=>{
...
test("Cuadrícula y Lista conservan exactamente filas, identidad, orden y conteos",()=>{
  const rows=[
...
test("loading, vacío, error y paginación son estados de datos, no de vista",()=>{
  ...
  for(const [rows,meta,expected] of cases){
    const a=View.dataContract(View.rowsForView(rows,"grid"),meta);
    const b=View.dataContract(View.rowsForView(rows,"list"),meta);
...
test("anomalías comparte vista pero conserva dataset e identidad independientes",()=>{
  ...
  assert.equal(h.target.attrs["data-informes-view"],"grid");
  assert.equal(h.anomalies.attrs["data-informes-view"],"grid");
  ...
  h.listeners.click({target:h.buttons[1],preventDefault(){},stopPropagation(){}});
  assert.equal(mounted.getView(),"list");
