 ...
 *
 * OJO con el alcance del histórico: GET /decisions solo devuelve las vivas y
 * las cerradas de la ÚLTIMA HORA (WHERE status='pending' OR decided_at > now-1h
 * OR deadline > now-1h). Para no perderlas, esta vista archiva en localStorage
 * todo cerrado que ve pasar (histórico LOCAL de este navegador). El histórico
 * de verdad exige ampliar el worker — misión aparte, aquí no se despliega. */
(function () {
  "use strict";
  ...
  // OJO: el worker NO expone chosen_by en GET /decisions, así que solo se sabe
  // el autor de las elecciones hechas desde este navegador (d.by, archivado en
  // local al pulsar). Sin dato NO se inventa: se dice «autor no registrado».
  function stamp(d) {
    var bits = [];
    ...
    if (d.status === "decided" || d.status === "cancelled") bits.push("<span>eligió <b>" + esc(d.by || "autor no registrado") + "</b></span>");
  ...
  // ── ARCHIVO LOCAL ───────────────────────────────────────────────────────────
  // El worker olvida las cerradas a la hora. Cada visita que ve pasar una
  // decisión cerrada la guarda aquí para que el histórico no se quede en blanco.
  // Es un apaño de navegador, no la verdad: el histórico real necesita worker.
  var LS_HIST = "yk_dec_hist_v1", LS_BY = "yk_dec_by_v1", HIST_MAX = 300;
  function lsGet(k) { try { return JSON.parse(localStorage.getItem(k) || "{}") || {}; } catch (e) { return {}; } }
  function lsSet(k, v) { try { localStorage.setItem(k, JSON.stringify(v)); } catch (e) {} }
  function closedAt(d) { return +d.decided_at || +d.deadline || +d.created_at || 0; }
  function archive(items) {
    var hist = lsGet(LS_HIST), by = lsGet(LS_BY), touched = false;
    (items || []).forEach(function (d) {
      if (!d || !d.id || d.status === "pending") return;
      var e = {}; Object.keys(d).forEach(function (k) { if (k !== "secondsLeft") e[k] = d[k]; });
      if (by[d.id]) e.by = by[d.id];
      hist[d.id] = e; touched = true;
    });
    if (!touched) return hist;
    var ids = Object.keys(hist).sort(function (a, b) { return closedAt(hist[b]) - closedAt(hist[a]); });
    if (ids.length > HIST_MAX) ids.slice(HIST_MAX).forEach(function (id) { delete hist[id]; });
    lsSet(LS_HIST, hist);
    return hist;
  }
  function rememberChooser(id, who) { var by = lsGet(LS_BY); by[id] = who; lsSet(LS_BY, by); }
...
// "" y el primer render se saltaba, dejando la sección vacía sin su mensaje.
    var api = config.worker.replace(/\/$/, "") + "/decisions", decisions = [], sig = null, histSig = null;
    ...
    // Vista COMPLETA: vivas arriba, cerradas (worker + archivo local) abajo.
    function renderFull() {
      var live = decisions.filter(function (d) { return d.status === "pending"; });
      var hist = lsGet(LS_HIST);
      var closed = Object.keys(hist).map(function (id) { return hist[id]; })
      ...
      var hSig = closed.map(function (d) { return d.id + ":" + d.status + ":" + (d.by || ""); }).join("|");
      ...
      histList.innerHTML = (closed.length ? closed.map(function (d) { return card(d, {stamp: true}); }).join("")
        : "<p class=\"decs-empty\">Todavía no hay decisiones cerradas archivadas en este navegador.</p>")
        + "<p class=\"decs-note\">Alcance real del histórico: el worker solo devuelve las decisiones vivas y las cerradas de la última hora. Lo anterior que ves aquí es lo que este navegador llegó a archivar; en otro equipo la lista puede ser más corta. El histórico completo exige ampliar <code>GET /decisions</code>.</p>";
    }
    ...
    function render() { archive(decisions); return full ? renderFull() : renderLive(); }
    ...
    document.addEventListener("click", async function (e) { var b = e.target.closest(".dec-opt[data-dec]"); if (!b) return; b.closest(".dec-opts").querySelectorAll("button").forEach(function (x) { x.disabled = true; }); rememberChooser(b.dataset.dec, "Carlos"); try { await fetch(api + "/" + encodeURIComponent(b.dataset.dec) + "/choose", {method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({choice:+b.dataset.i,by:"Carlos"})}); } finally { load(); } });
