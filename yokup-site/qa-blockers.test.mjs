...
test("Informes recompone identidad física y respeta identidad del worker",async()=>{
  ...
  assert.match(pdf,/reportDisplay\(rawAgent,machine\)/);
});
test("Objetivos conserva creación diaria y bulk canónico en ambas densidades",async()=>{const s=await read("./objetivos.html");assert.match(s,/scheduled_for:\$\("#boardDay"\)\.value\|\|TODAY/);assert.match(s,/body:JSON\.stringify\(\{ids,status\}\)/);assert.match(s,/confirm\("Cambiar "/);assert.match(s,/function objectiveRow[\s\S]*data-bulk-id/);assert.match(s,/density-compact[\s\S]*density-comfortable/);});
