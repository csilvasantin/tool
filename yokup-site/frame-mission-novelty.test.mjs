...
test("cursor detecta +1 y saltos múltiples aunque cambien los totales",()=>{
  ...
  result=tracker.observe(payload(14,{curso:1,pend:0}));
...
test("unread persiste tras reload y sólo ack explícito lo consume",()=>{
  const store=memory();let publishes=0,first=core(store,()=>publishes++);first.observe(payload(7));first.observe(payload(9));
